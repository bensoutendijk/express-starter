...
router.get('/current', ...auth.required, async (req: Request, res) => {
  const { user } = req;
  ...
  res.cookie('token', `Token ${localUser.generateHttpOnlyJWT()}`, {
    expires: new Date(Date.now() + 1000 * 60 * 30),
    httpOnly: true,
  });
  res.cookie('token2', `Token ${localUser.generateJWT()}`, {
    expires: new Date(Date.now() + 1000 * 60 * 30),
  });
